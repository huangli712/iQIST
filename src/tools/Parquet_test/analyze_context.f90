...
! irreducible singlet/triplet vertex Gamma
     complex(dp), public, save, allocatable :: gamat_s(:,:)
     complex(dp), public, save, allocatable :: gamat_t(:,:)
...
!!	 nt_2p_q = 4*ktime*ktime
	 ...
	 allocate(gamat_c(4*ktime*ktime*nffrq,4*ktime*ktime*nffrq), stat=istat)
         ...
         allocate(gamat_t(4*ktime*ktime*nffrq,4*ktime*ktime*nffrq), stat=istat)
...
!!         if ( allocated(Fc_pp_t) )    deallocate(Fc_pp_t)
	 ...
	 if ( allocated(gamat_t))    deallocate(gamat_t)
