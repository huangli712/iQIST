...
!!	this is the parameter set of the t2g three bangs model
	t1=0.5; t2=0.05; t3=0.4; t4=0.15; t5=0.06;
...
!! Here we also have the 3 bands t2g dispersion
		Exy = -2.0d0*t3*(cos(kx)+cos(ky))-4.0d0*t4*cos(kx)*cos(ky)-2.0d0*t5*(cos(2*kx)+cos(2*ky)) 
		Ezx = -2.0d0*t1*cos(kx)-2.0d0*t2*cos(ky)
		Eyz = -2.0d0*t1*cos(ky)-2.0d0*t2*cos(kx)
...
!! prepare the chi0_dm sum_{P}G(P+Q)G(P)
!! prepare the chi0_st sum_{P}G(P+Q)G(-P)
...
!!	phi_pp_t(:,:,:,:,:) = phi_pp_t(:,:,:,:,:)/(real(nffrq*nffrq,dp))
	...
	open(mytmp, file='analyze.twop.phi.ph.c.m.1st.order.dat', form='formatted', status='unknown')
	open(mytmp1, file='analyze.twop.phi.pp.s.t.1st.order.dat', form='formatted', status='unknown')
	do l=-nffrq/2+1, nffrq/2 ! iw
	do m=-nffrq/2+1, nffrq/2 ! iw'
!!	do i=-ktime+1,ktime
	i=0
!!	do j=-ktime+1,ktime
	j=0
	  kx = rDk*pi*(real(i,dp)-half)
	  ky = rDk*pi*(real(j,dp)-half)
	  write(mytmp,'(2i6,4f16.8)') l, m, phi_ph_c(i,j,0,l,m), phi_ph_m(i,j,0,l,m)
	  write(mytmp1,'(2i6,4f16.8)') l, m, phi_pp_s(i,j,0,l,m), phi_pp_t(i,j,0,l,m)
	end do ! over l={-nffrq/2+1,nffrq/2} loop
	end do ! over m={-nffrq/2+1,nffrq/2} loop
	 close(mytmp)
	 close(mytmp1)
...
!    Gamma_s(k,k',w,w') = lambda_s(w,w')+(1/2)(Phid1 + Phid2)-(3/2)(Phim1 + Phim2),
!    where
!    Phi(d/m)1(q=0,nu=0)_p,p' = Phi(d/m)1(w'-w,k'-k)_-w',w
!    Phi(d/m)2(q=0,nu=0)_p,p' = Phi(d/m)2(w'+w,k'+k)_-w',-w
!
!    Fix q and nu, and
!    Gamma_m(k,k',q,w,w',v=0) = lambda_m(w,w')-(1/2)Phid1+(1/2)Phim1-(1/2)Phis2+(1/2)Phit2
!    where
!    Phi(d/m)1(q,nu)_p,p' = Phi(d/m)1(w'-w,k'-k)_w,w+nu
!    Phi(s/t)2(q,nu)_p,p' = Phi(s/t)2(w+w'+nu,k+k'+q)_-w-nu,-w
...
!!	construct the particle-hole magnetic irreducible vertex
          ...
          gamat_t(p1,p2)= lambda_t(l,m) - 0.5d0*phi_ph_c(iqx1,iqy1,inu,-m+1,l) - 0.5d0*phi_ph_m(iqx1,iqy1,inu,-m+1,l)
          gamat_s(p1,p2)= lambda_s(l,m) - 0.5d0*phi_ph_c(iqx1,iqy1,inu,-m+1,l) + 1.5d0*phi_ph_m(iqx1,iqy1,inu,-m+1,l)
          else if(m+l-1.eq.inu) then  ! Phi(d/m)2(w'+w,k'+k)_-w',-w ! Note that the index of w+w' is (l+m-1)
          ...
          gamat_t(p1,p2)= lambda_t(l,m) + 0.5d0*phi_ph_c(iqx2,iqy2,inu,-m+1,-l+1) + 0.5d0*phi_ph_m(iqx2,iqy2,inu,-m+1,-l+1)
          gamat_s(p1,p2)= lambda_s(l,m) - 0.5d0*phi_ph_c(iqx2,iqy2,inu,-m+1,-l+1) + 1.5d0*phi_ph_m(iqx2,iqy2,inu,-m+1,-l+1)
          end if 
...
!!          end if
        ...
        write(*,*) "finish analyze_calculate 1st order: Gamma_m, Gamma_c, Gamma_s, Gamma_t !!!"
...
! loop index over momentum, frequency, orbital spaces
      ...
      integer :: i3,j4,l3,m4,p3,p4,ikx3,iky3,ikx4,iky4,ipqx,ipqy,p1q,p1qm
...
! dummy two-particle, intermediate, pp_singlet
		complex(dp), allocatable :: pp_singlet(:,:), phi_pp_s_k_kp(:,:), phi_pp_s_k_kp_1(:,:)
! dummy two-particle, intermediate, pp_triplet
		complex(dp), allocatable :: pp_triplet(:,:), phi_pp_t_k_kp(:,:), phi_pp_t_k_kp_1(:,:)
...
! vertex ladder, phi_pp_s_k_kp_q
		complex(dp), allocatable :: phi_pp_s_k_kp_q(:,:,:)
! vertex ladder, phi_pp_t_k_kp_q
		complex(dp), allocatable :: phi_pp_t_k_kp_q(:,:,:)
	
! irreducible vertext, gamat_c_k_kp_q
		complex(dp), allocatable :: gamat_c_k_kp_q(:,:,:)
! irreducible vertext, gamat_m_k_kp_q
		complex(dp), allocatable :: gamat_m_k_kp_q(:,:,:)
! irreducible vertext, gamat_s_k_kp_q
		complex(dp), allocatable :: gamat_s_k_kp_q(:,:,:)
! irreducible vertext, gamat_t_k_kp_q
		complex(dp), allocatable :: gamat_t_k_kp_q(:,:,:)
...
! allocate memory
	...
	allocate(pp_singlet(4*ktime*ktime*nffrq,4*ktime*ktime*nffrq), stat=istat)
	allocate(phi_pp_s_k_kp(4*ktime*ktime*nffrq,4*ktime*ktime*nffrq), stat=istat)
	allocate(phi_pp_s_k_kp_1(4*ktime*ktime*nffrq,4*ktime*ktime*nffrq), stat=istat)
	allocate(phi_pp_s_k_kp_q(4*ktime*ktime*nffrq,4*ktime*ktime*nffrq,4*ktime*ktime*(2*nbfrq-1)), stat=istat)
	
	allocate(pp_triplet(4*ktime*ktime*nffrq,4*ktime*ktime*nffrq), stat=istat)
	allocate(phi_pp_t_k_kp(4*ktime*ktime*nffrq,4*ktime*ktime*nffrq), stat=istat)
	allocate(phi_pp_t_k_kp_1(4*ktime*ktime*nffrq,4*ktime*ktime*nffrq), stat=istat)
	allocate(phi_pp_t_k_kp_q(4*ktime*ktime*nffrq,4*ktime*ktime*nffrq,4*ktime*ktime*(2*nbfrq-1)), stat=istat)
	
	allocate(gamat_c_k_kp_q(4*ktime*ktime*nffrq,4*ktime*ktime*nffrq,4*ktime*ktime*(2*nbfrq-1)), stat=istat)
	allocate(gamat_m_k_kp_q(4*ktime*ktime*nffrq,4*ktime*ktime*nffrq,4*ktime*ktime*(2*nbfrq-1)), stat=istat)
	allocate(gamat_s_k_kp_q(4*ktime*ktime*nffrq,4*ktime*ktime*nffrq,4*ktime*ktime*(2*nbfrq-1)), stat=istat)
	allocate(gamat_t_k_kp_q(4*ktime*ktime*nffrq,4*ktime*ktime*nffrq,4*ktime*ktime*(2*nbfrq-1)), stat=istat)
	...
	pp_singlet(:,:) = czero
	phi_pp_s_k_kp(:,:) = czero
	phi_pp_s_k_kp_1(:,:) = czero
	phi_pp_s_k_kp_q(:,:,:) = czero
	
	pp_triplet(:,:) = czero
	phi_pp_t_k_kp(:,:) = czero
	phi_pp_t_k_kp_1(:,:) = czero
	phi_pp_t_k_kp_q(:,:,:) = czero
...
!! prepare the momentum-dependence of ladder phi_pp_s_k_kp(k,w,k',w',q,v)
!! prepare the momentum-dependence of ladder phi_pp_t_k_kp(k,w,k',w',q,v)
...
!  Step 1a: Prepare 1/chi_ph_k_kp(k,w,k',w',q,nu) = 1/G(k+q,w+nu)G(k,w) - gamma_ph_X(k,w,k',w',q=0,nu=0)
!  Step 1b: Prepare 1/chi_pp_k_kp(k,w,k',w',q,nu) = 1/G(k+q,w+nu)G(-k,-w) - gamma_pp_X(k,w,k',w',q=0,nu=0)
		...
		iky2=((j-1)/(2*ktime) + 1) - ktime  ! ky': (-ktime+1:ktime)
		  ...
		  pp_singlet(p1,p2) = -gamat_s(p1,p2)
		  pp_triplet(p1,p2) = -gamat_t(p1,p2)
		  
		if ((i.eq.j).and.(l.eq.m)) then
		    ph_magnet(p1,p2) = -cone*beta/(grnf_lattice(iqx1,iqy1,l+inu)*grnf_lattice(ikx1,iky1,l)) + ph_magnet(p1,p2)
		    !!cone/chi0_dm(iqx,iqy,l,inu) + ph_magnet(p1,p2)
		    ph_charge(p1,p2) = -cone*beta/(grnf_lattice(iqx1,iqy1,l+inu)*grnf_lattice(ikx1,iky1,l)) + ph_charge(p1,p2)
		    !!cone/chi0_st(iqx,iqy,l,inu) + pp_singlet(p1,p2)
		    pp_singlet(p1,p2) = 2.d0*cone*beta/(grnf_lattice(iqx1,iqy1,l+inu)*grnf_lattice(-ikx1+1,-iky1+1,-l+1)) + pp_singlet(p1,p2)
		    pp_triplet(p1,p2) = 2.d0*cone*beta/(grnf_lattice(iqx1,iqy1,l+inu)*grnf_lattice(-ikx1+1,-iky1+1,-l+1)) + pp_triplet(p1,p2)
		end if
	    ...
	    call analyze_zmat_inv(4*ktime*ktime*nffrq,pp_singlet)
	    call analyze_zmat_inv(4*ktime*ktime*nffrq,pp_triplet)
...
!  Step 2: sum_{k1,w1,k2,w2} gamma_ph_X{k,w,k1,w1} * chi_ph_X(inu)_{qx,qy,k1,w1,k2,w2} * gamma_ph_X(k2,w2,k,w')
	    ...
	    call analyze_zmat_prd(4*ktime**2*nffrq,gamat_s,pp_singlet,phi_pp_s_k_kp_1)
	    call analyze_zmat_prd(4*ktime**2*nffrq,phi_pp_s_k_kp_1,gamat_s,phi_pp_s_k_kp)
	    
	    call analyze_zmat_prd(4*ktime**2*nffrq,gamat_t,pp_triplet,phi_pp_t_k_kp_1)
	    call analyze_zmat_prd(4*ktime**2*nffrq,phi_pp_t_k_kp_1,gamat_t,phi_pp_t_k_kp)
	    
	    phi_ph_m_k_kp_q(:,:,q1) = -phi_ph_m_k_kp(:,:)
	    phi_ph_c_k_kp_q(:,:,q1) = -phi_ph_c_k_kp(:,:)
	    phi_pp_s_k_kp_q(:,:,q1) = -phi_pp_s_k_kp(:,:)
	    phi_pp_t_k_kp_q(:,:,q1) = -phi_pp_t_k_kp(:,:)
	    
	end do ! over inu={-nbfrq+1,nbfrq-1}
	end do ! over q={1,4*ktime**2} loop
	
!!          phi_ph_c_k_kp_q(:,:,:) = phi_ph_c_k_kp_q(:,:,:)/(4*ktime**2*nffrq*4*ktime**2*nffrq)
!!          phi_ph_m_k_kp_q(:,:,:) = phi_ph_m_k_kp_q(:,:,:)/(4*ktime**2*nffrq*4*ktime**2*nffrq)
!!          phi_pp_s_k_kp_q(:,:,:) = phi_pp_s_k_kp_q(:,:,:)/(4*ktime**2*nffrq*4*ktime**2*nffrq)
!!          phi_pp_t_k_kp_q(:,:,:) = phi_pp_t_k_kp_q(:,:,:)/(4*ktime**2*nffrq*4*ktime**2*nffrq)
        
        write(*,*) "finish analyze_calculate_vertexladder_2nd_order!!!"
        
        open(mytmp, file='analyze.twop.phi.ph.c.m.2nd.order.dat', form='formatted', status='unknown')
        open(mytmp1, file='analyze.twop.phi.pp.s.t.2nd.order.dat', form='formatted', status='unknown')
!!        do q=1,4*ktime**2		 ! q  = (qx,qy)
	  q=2*ktime**2 - ktime		 ! this is for q=(0,0)
	  iqx=(mod(q-1,2*ktime) + 1) - ktime ! qx : (-ktime+1:ktime)
	  iqy=((q-1)/(2*ktime) + 1) - ktime  ! qy : (-ktime+1:ktime)
...
!!	    do i=1,4*ktime**2         ! k = (kx,ky)
!!		i=2*ktime**2 - ktime	! this is for q=(0,0)
		i=4*ktime**2		! this is for q=(pi,pi)
		ikx1=(mod(i-1,2*ktime) + 1) - ktime ! kx: (-ktime+1:ktime)
		iky1=((i-1)/(2*ktime) + 1) - ktime  ! ky: (-ktime+1:ktime)
!!	    do j=1,4*ktime**2         ! k' = (kx',ky')
!!		j=2*ktime**2 - ktime	! this is for q=(0,0)
		j=4*ktime**2		! this is for q=(pi,pi)
		ikx2=(mod(j-1,2*ktime) + 1) - ktime ! kx': (-ktime+1:ktime)
		iky2=((j-1)/(2*ktime) + 1) - ktime  ! ky': (-ktime+1:ktime)
...
!!		  m=1
	  ...
	  write(mytmp,'(2i6,4f16.8)') l, m, phi_ph_c_k_kp_q(p1,p2,q1), phi_ph_m_k_kp_q(p1,p2,q1)
	  write(mytmp1,'(2i6,4f16.8)') l, m, phi_pp_s_k_kp_q(p1,p2,q1), phi_pp_t_k_kp_q(p1,p2,q1)
	      end do ! over l={-nffrq/2+1,nffrq/2} loop
	      end do ! over m={-nffrq/2+1,nffrq/2} loop
...
!!      iky3 = ((k-1)/(2*ktime) + 1) - ktime  ! qy : (-ktime+1:ktime)
       ...
       iqy1=add_k_1D(iky2 - iky1) ! ky'-ky
       iqx2=add_k_1D(ikx2 + ikx1) ! kx'+kx
       iqy2=add_k_1D(iky2 + iky1) ! ky'+ky
       
!!       iqx2=add_k_1D(ikx2 + ikx1 + ikx3) ! kx'+kx+qx
!!       iqy2=add_k_1D(iky2 + iky1 + iky3) ! ky'+ky+qy     
!!       ipqx=add_k_1D(ikx1 + ikx3) ! kx + qx
!!       ipqy=add_k_1D(iky1 + iky3) ! ky + qy
...
!!          p1q = (((ipqx+ktime) + (ipqy+ktime-1)*2*ktime)-1)*nffrq + l + nffrq/2 ! p1+q
!!          p1qm = (4*ktime**2+1-((ipqx+ktime) + (ipqy+ktime-1)*2*ktime)-1)*nffrq -l+1 + nffrq/2 ! -(p1+q)
          do m=-nffrq/2+1,nffrq/2   ! w'= (-3,-2,...,3,4)
          p2 = (j-1)*nffrq + m + nffrq/2 ! p2 = (k',w')=(1,2,...,4*ktime**2*nffrq)
...
!!	singlet
	    gamat_s(p1,p2)= lambda_s(l,m) - 0.5d0*phi_ph_c_k_kp_q(p2m,p1,q1) + 1.5d0*phi_ph_m_k_kp_q(p2m,p1,q1)
!!	triplet
	    gamat_t(p1,p2)= lambda_t(l,m) - 0.5d0*phi_ph_c_k_kp_q(p2m,p1,q1) - 0.5d0*phi_ph_m_k_kp_q(p2m,p1,q1)
!!	magnetic
!!	    gamat_m(p1,p2)= lambda_m(l,m) + 0.5d0*phi_ph_c_k_kp_q(p1,p1q,q1) - 0.5d0*phi_ph_m_k_kp_q(p1,p1q,q1)
	    
	    else if (((m+l-1).le.(nbfrq-1)).and.((m+l-1).ge.(-nbfrq+1))) then
	    q2 = (((iqx2+ktime) + (iqy2+ktime-1)*2*ktime)-1)*(2*nbfrq-1) + (m+l-1) + nbfrq  ! q2 = p2+p1+q = (k'+k+q, w'+w)
!!	singlet	    
           gamat_s(p1,p2)= lambda_s(l,m)- 0.5d0*phi_ph_c_k_kp_q(p2m,p1m,q2) + 1.5d0*phi_ph_m_k_kp_q(p2m,p1m,q2)
!!	triplet
	   gamat_t(p1,p2)= lambda_t(l,m) + 0.5d0*phi_ph_c_k_kp_q(p2m,p1m,q2) + 0.5d0*phi_ph_m_k_kp_q(p2m,p1m,q2)
!!	magnetic
!!	    gamat_m(p1,p2)= lambda_m(l,m) + 0.5d0*phi_pp_s_k_kp_q(p1qm,p1m,q2) - 0.5d0*phi_pp_t_k_kp_q(p1qm,p1m,q2)
...
!!       end do ! over inu={-nbfrq+1,nbfrq-1} loop
	...
	deallocate(ph_magnet)
        ...
        deallocate(pp_singlet)
        deallocate(phi_pp_s_k_kp)
        deallocate(phi_pp_s_k_kp_1)
        deallocate(phi_pp_s_k_kp_q)
        
        deallocate(pp_triplet)
        deallocate(phi_pp_t_k_kp)
        deallocate(phi_pp_t_k_kp_1)
        deallocate(phi_pp_t_k_kp_q)
...
!!          Gamat_m(p1,p2) = -Gamat_m(p1,p2)*grnf_lattice(iqx,iqy,jw)*grnf_lattice(ikx2,iky2,jw)/(beta*real((2*ktime)**2,dp))
          
!!	this is the calculation for pairing LEVs
	  Gamat_t(p1,p2) = -Gamat_t(p1,p2)*grnf_lattice(ikx2,iky2,jw)*grnf_lattice(-ikx2+1,-iky2+1,-jw+1)/(beta*real((2*ktime)**2,dp))
...
! Calculate the eigenvalues and vectors of Gamat_s: 
          ...
          call zgeev('N','V',nt_2p,Gamat_t,nt_2p,Eval,Umat,nt_2p, &
