...
! update the expt_v and expt_t, matrix of time evolution operator
     if ( is == nsize ) then
         expt_t(ilast) = t_next - t_prev
     else
         expt_v(ilast) = t_next - t_prev
         expt_t(1) = expt_t(2)
     endif ! back if ( is == nsize ) block
...
! so we store the rightmost time evolution operator at expt_t
     if ( is == nsize ) then
         expt_t(2 ) = t_next - t_prev
     else
         as = index_v(is)
         expt_v(as) = t_next - t_prev
     endif ! back if ( is == nsize ) block
...
! number of non-zero element in propagated state v
     integer  :: vz
...
! imaginary time interval for current operator
     real(dp) :: dtau

! scaling factor for lvec, which is used to avoid the numerical instability
     real(dp) :: raux

! total scaling factor for lvec, only the exponent number part
     real(dp) :: rexp

! left eigenvectors of local hamiltonian matrix
     real(dp) :: lvec(ncfgs)
...
! reset hmat to zero
! note: here we use hmat as a dummy matrix, we do not store the local
...
! loop over selected eigenstates
         EIGENVEC_LOOP1: &
         do i=1,nvect
! get corresponding eigenstates
             lvec = vmat(:,i) ! rvec = wmat(i,:)

! loop over all the existing operators, we perform multiplication forward
             OPERATOR_LOOP1: &
             do j=1,csize
! get the type, flavor, and imaginary time interval for current operator 
                 vt   = type_v ( index_t(j) )
                 vf   = flvr_v ( index_t(j) )
                 dtau = expt_v ( index_t(j) )

! calculate the multiplication of time evolution operator and a propagated
! state using real leja points method.
!-------------------------------------------------------------------------
! if it is the initial state, it is simply multiplied with an exponential
! factor since it is an eigenstate of the local hamiltonian matrix
                 if ( j == 1 ) then
                     mvec = lvec * exp( - dtau * eigs(i) )
                 else
! check if the propagated state is dead, if not, perform real leja points
! interpolation, if yes, then exit internal loop
                     vz = count ( lvec /= zero ) ! get number of non-zero elements
                     if ( vz > 0 ) then ! the propagated state is alive
                         call leja_dsymv( dtau, lvec, mvec )
                     else               ! the propagated state is dead
                         EXIT OPERATOR_LOOP1
                     endif ! back if ( vz > 0 ) block
...
! please pay special attention to the null state
! check the absolute maximum value
                 raux = maxval( abs( lvec ) )
! the propagated state is not null
                 if ( raux > zero ) then
! record the scaling factor
                     rexp = rexp + log(raux)
! scale it
                     lvec = lvec / raux
                 endif ! back if ( raux > zero ) block
             enddo OPERATOR_LOOP1 ! over j={1,csize} loop

! determine the final imaginary time interval, it depends on the calculation mode
             if ( cmode == 1 ) then
                 dtau = expt_t(1)
             else
                 dtau = expt_t(2)
...
! calculate the multiplication of the final time evolution operator and a
! propagated state using real leja points method.
!-------------------------------------------------------------------------
! if it is the initial state, it is simply multiplied with an exponential
! factor since it is an eigenstate of the local hamiltonian matrix
             if ( csize == 0 ) then
...
! add the contribution of current eigenstates to hmat
! note: mvec must be multiplied with the rexp, and beta * Uc is the global
! scaling factor to avoid \infity case
             if ( csize > 0 ) then
                 mvec = mvec * exp ( rexp + beta / two * U )
             endif ! back if ( csize > 0 ) block
! note: hmat only contain the contributions from the selected nvect eigenstates
             do j=1,nvect
                 hmat(j,i) = dot_product( wmat(j,:), mvec )
             enddo ! over j={1,nvect} loop
! do not forget to reset rexp for next selected eigenstate
             rexp = zero
...
!-------------------------------------------------------------------------
! case B: partly-normal or fully-normal mode
!-------------------------------------------------------------------------
     else if ( cmode == 2 .or. cmode == 4 ) then
...
! scale the propagated state to overcome the divergence problem
! please special attention to the null state
! check the absolute maximum value
                 raux = maxval( abs( lvec ) )
! the propagated state is not null
                 if ( raux > zero ) then
...
! scale it
             ...
             enddo operator_loop2 ! over j={1,csize} loop

! determine the final imaginary time interval
             dtau = expt_t(2)
...
! display the operators (flavor part)
         ...
         do j=1,nsize
             ...
             write(mystd,'(4X,i4,f12.6)') j, expt_v( index_v(j) )
         ...
         write(mystd,'(4X,a)') '< diag >'
         write(mystd,'(4X,i4,f12.6)') 9999, expt_t(2)
         write(mystd,*) ! write empty line
