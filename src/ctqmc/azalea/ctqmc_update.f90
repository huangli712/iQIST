...
! recalculate Dmm from scratch
     do j=1,kaux
         ...
         do i=1,kaux
             ...
             if ( tau_start < tau_end ) then
                 Dmm(i, j) = -ctqmc_make_htau(flvr, tau_start - tau_end + beta)
             else
                 Dmm(i, j) =  ctqmc_make_htau(flvr, tau_start - tau_end)
             endif ! back if ( tau_start < tau_end ) block
         enddo ! over i={1,kaux} loop
     enddo ! over j={1,kaux} loop

! calculate Tmm: Tmm = Dmm . Mmat
     call dgemm('N', 'N', kaux, kaux, kaux, one, Dmm, kaux, mmat(1:kaux, 1:kaux, flvr), kaux, zero, Tmm, kaux)
