...
! obtain current date and time
     ...
     write(mystd,'(2X,a)') 'Version: 2014.09.24T '//'(built at '//__TIME__//" "//__DATE__//')'
...
# if defined (MPI)
     ...
     write(mystd,'(2X,a,i4)') 'GARDENIA >>> parallelism: Yes >>> processors:', nprocs

# else   /* MPI */

     write(mystd,'(2X,a,i4)') 'GARDENIA >>> parallelism: No  >>> processors:', 1
...
! obtain current date and time
     ...
     write(mystd,'(2X,a)') 'GARDENIA >>> I am tired and want to go to bed. Bye!'
     write(mystd,'(2X,a)') 'GARDENIA >>> happy ending at '//date_time_string
...
!!>>> ctqmc_print_summary: print the running parameters, only for reference
  subroutine ctqmc_print_summary()
     ...
     write(mystd,'(2X,a)') 'GARDENIA >>> parameters list:'
...
! about iteration number
     write(mystd,'(2X,a,i3,2(a,i10))') 'GARDENIA >>> iter:', iter, ' sweep:', cstep, ' of ', nsweep
...
! about reswap action
     ...
     write(mystd,'(4X,a,3i10)')   'count:', int( reswap_tcount ), int( reswap_accept ), int( reswap_reject )
     write(mystd,'(4X,a,3f10.5)') 'ratio:', one, reswap_accept / reswap_tcount, reswap_reject / reswap_tcount
