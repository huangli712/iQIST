...
!!>>> cat_make_trace: calculate the contribution to final trace for
!!>>> a given string
  subroutine cat_make_trace2(csize, string, index_loc, expt_loc, trace)
     implicit none
...
! whether we should determine the order
     logical  :: is_order

! real(dp) dummy matrices
     real(dp) :: mat_a(max_dim_sect,max_dim_sect)
     real(dp) :: mat_b(max_dim_sect,max_dim_sect)
     real(dp) :: mat_c(max_dim_sect,max_dim_sect)
     real(dp) :: mat_r(max_dim_sect,max_dim_sect)
     real(dp) :: mat_t(max_dim_sect,max_dim_sect)
...
! loop over all the fermion operators in this part
             do j=ops(i),ope(i),2
                 indx = sectors( string(j)   )%istart
                 dim_bm = sectors( string(j+1)   )%ndim
                 dim_bn = sectors( string(j)   )%ndim
                 vt = type_v( index_loc(j) )
                 vf = flvr_v( index_loc(j) )
                 do k=1,dim_bn
                     mat_b(1:dim_bm,k) = sectors(string(j)%fmat(vf,vt)%val(1:dim_bm,k) * expt_v(indx+k-1,index_loc(j))
                 enddo ! over l={1,dim4} loop
...
! calculate the trace and store the final product
     ...
     return
  end subroutine cat_make_trace2
